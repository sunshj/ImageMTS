...
class Ui_RenameWindow(QMainWindow):
    ...
    def setupUi(self, MainWindow):
        ...
        self.label.setGeometry(QtCore.QRect(15, 90, 70, 21))
        self.label.setObjectName("label")
        # 起始编号标签
        self.label_2 = QtWidgets.QLabel(self.groupBox)
        self.label_2.setGeometry(QtCore.QRect(210, 90, 65, 21))
        ...
        self.label_3.setGeometry(QtCore.QRect(350, 90, 65, 21))
        ...
        self.label_4.setGeometry(QtCore.QRect(15, 20, 100, 21))
        self.label_4.setObjectName("label_4")
        # 图片路径文本框
        self.lineEdit_4 = QtWidgets.QLineEdit(self.groupBox_2)
        self.lineEdit_4.setGeometry(QtCore.QRect(115, 20, 201, 21))
        ...
        self.tableWidget.setVerticalScrollBarPolicy(
            QtCore.Qt.ScrollBarAlwaysOn)  # 垂直滚动条
    ...
    def retranslateUi(self, MainWindow):
        ...
        MainWindow.setWindowIcon(QIcon('img/rename.png'))
                ...
                if os.path.isfile(filepath):  # 判断是否为文件
                    ...
                    if imageMark.Ui_MarkWindow().isImg(imgType):  # 判断是否为图片
                        ...
                        self.tableWidget.setItem(
                            i, 0, QtWidgets.QTableWidgetItem(self.list[i]))
                        # 设置第二列的值为文件路径
                        self.tableWidget.setItem(
                            i, 1, QtWidgets.QTableWidgetItem(self.img_path))
    ...
    def reName(self):
        ...
        try:
            for i in range(self.tableWidget.rowCount()):  # 遍历图片列表
                ...
                filepath = os.path.join(
                    self.img_path, filename)  # 记录遍历到的文件名(包括路径)
                if os.path.isfile(filepath):  # 判断是否为文件
                    imgType = os.path.splitext(filepath)[1]  # 获取扩展名
                    if imageMark.Ui_MarkWindow().isImg(imgType):  # 判断是否为图片
                        if self.radioButton.isChecked():  # 文件名大写
                            ...
                            newfilepath = os.path.join(
                                self.img_path, newfilename)  # 新文件名（包括路径）
                            ...
                            # 根据模板、起始编号和增量值生成新文件名
                            newfilename = strid[0:4] + \
                                template.format(id+step*i)+imgType
                            newfilepath = os.path.join(
                                self.img_path, newfilename)   # 新文件名（包括路径）
                            os.rename(filepath, newfilepath)  # 重命名文件
                        num += 1  # 数量加1
                        # 更新第一列的值为重命名之后的图片文件名
                        self.tableWidget.setItem(
                            i, 0, QtWidgets.QTableWidgetItem(newfilename))
            self.statusbar.showMessage(
                '批量重命名完成，共处理图片 ' + str(num) + ' 张')  # 状态栏中显示处理图片总数
...
if __name__ == '__main__':
    ...
    RenameWindow = QtWidgets.QMainWindow()  # 创建窗体对象
    ui = Ui_RenameWindow()  # 创建PyQt5设计的窗体对象
    ui.setupUi(RenameWindow)  # 调用PyQt5窗体的方法对窗体对象进行初始化设置
    RenameWindow.show()  # 显示窗体
