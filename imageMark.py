...
from PyQt5.QtGui import QFontMetrics, QFontInfo, QIcon
...
class Ui_MarkWindow(QMainWindow):
    ...
    def setupUi(self, MainWindow):
        ...
        self.listWidget.setGeometry(QtCore.QRect(2, 2, 141, 391))
        ...
        self.radioButton.setGeometry(QtCore.QRect(20, 20, 120, 16))
        self.radioButton.setChecked(True)  # 默认选中
        self.radioButton.setObjectName("radioButton")

        self.label = QtWidgets.QLabel(self.groupBox)
        self.label.setGeometry(QtCore.QRect(30, 50, 64, 16))
        self.label.setObjectName("label")

        # 设置要输入水印文字的文本框
        self.lineEdit = QtWidgets.QLineEdit(self.groupBox)
        self.lineEdit.setGeometry(QtCore.QRect(100, 50, 231, 20))
        ...
        self.radioButton_2.setGeometry(QtCore.QRect(20, 80, 120, 16))
        ...
        self.label_2.setGeometry(QtCore.QRect(30, 110, 64, 16))
        self.label_2.setObjectName("label_2")

        # 设置显示水印图片路径的文本框
        self.lineEdit_2 = QtWidgets.QLineEdit(self.groupBox)
        self.lineEdit_2.setGeometry(QtCore.QRect(100, 110, 231, 20))
        ...
        self.label_3.setGeometry(QtCore.QRect(255, 31, 65, 21))
        ...
        self.comboBox.addItem('左上')
        self.comboBox.addItem('右上')
        self.comboBox.addItem('左下')
        self.comboBox.addItem('右下')
        self.comboBox.addItem('居中')
        ...
        self.horizontalSlider.setGeometry(QtCore.QRect(75, 30, 170, 22))
    ...
    def retranslateUi(self, MainWindow):
        ...
        MainWindow.setWindowTitle(_translate("MainWindow", "图片批量加水印"))
        MainWindow.setWindowIcon(QIcon('img/mark.png'))
        ...
        try:
            ...
            for i in range(0, len(self.list)):  # 遍历图片列表
                filepath = os.path.join(
                    self.img_path, self.list[i])  # 记录遍历到的文件名
    ...
    def setImg(self):
        try:
            ...
            self.waterimg = QFileDialog.getOpenFileName(
                None, '选择水印图片',  "图片文件(*.jpeg;*.png;*.jpg;*.bmp)")
    ...
    def textMark(self, img, newImgPath):
        try:
            ...
            newImg = Image.new(
                'RGBA', im.size, (255, 255, 255, 0))  # 存储添加水印后的图片
            ...
            # 设置水印文字位置
            if self.comboBox.currentText() == '左上':
                position = (0, 0)
            elif self.comboBox.currentText() == '左下':
                position = (0, imgheight - txtheight)
            elif self.comboBox.currentText() == '右上':
                position = (imgwidth - txtwidth, 0)
            elif self.comboBox.currentText() == '右下':
                position = (imgwidth - txtwidth, imgheight - txtheight)
            elif self.comboBox.currentText() == '居中':
                position = (imgwidth/2, imgheight/2)
            # 设置文本颜色
            imagedraw.text(position, self.lineEdit.text(),
                           font=font, fill="#FCA454")
            # 设置透明度
    ...
    def imgMark(self, img, newImgPath):
        ...
        newsize = (int(nimgwidth * markscale),
                   int(nimgheight * markscale))  # 计算新的尺寸大小
        rgbamark = rgbamark.resize(
            newsize, resample=Image.ANTIALIAS)  # 重新设置水印图片大小
        ...
        # 水印位置
        rgbaim.paste(rgbamark,  rgbamarkpha)
        try:
    ...
    def addMark(self):
        if self.lineEdit_3.text() == '':  # 判断是否选择了保存路径
            ...
            for i in range(0, self.listWidget.count()):  # 遍历图片列表
                # 设置原始图片路径（包括文件名）
                filepath = os.path.join(
                    self.img_path, self.listWidget.item(i).text())
                # 设置水印图片保存路径（包括文件名）
                newfilepath = os.path.join(
                    self.lineEdit_3.text(), self.listWidget.item(i).text())
                if self.radioButton.isChecked():  # 判断是否选择文字水印单选按钮
                    if self.lineEdit.text() == '':  # 判断是否输入了水印文字
                        QMessageBox.warning(
                            None, '警告', '请输入水印文字', QMessageBox.Ok)
                        return
                    else:
                        # 调用textMark方法添加文字水印
                    ...
                    if self.lineEdit_2.text() != '':  # 判断水印图片不为空
                        # 调用imgMark方法添加图片水印
                        self.imgMark(filepath, newfilepath)
                        num += 1  # 处理图片数量加1
                    else:
                        QMessageBox.warning(
                            None, '警告', '请选择水印图片', QMessageBox.Ok)
            self.statusBar.showMessage(
                '任务完成，此次共处理 ' + str(num) + ' 张图片')  # 显示处理图片总数


if __name__ == '__main__':
    import sys
    app = QtWidgets.QApplication(sys.argv)
    MarkWindow = QtWidgets.QMainWindow()  # 创建窗体对象
    ui = Ui_MarkWindow()  # 创建PyQt5设计的窗体对象
    ui.setupUi(MarkWindow)  # 调用PyQt5窗体的方法对窗体对象进行初始化设置
    MarkWindow.show()  # 显示窗体
